...
const sageMakerRuntime = new AWS.SageMakerRuntime({ region: config.AWS_REGION })
 ...
 * @return {string}
 */
const createRequestBody = (challenge) => {
  const data = new Map()
  ...
  data.set('Challenge Manager', 'GunaK-TopCoder')
  data.set('Challenge Copilot', 'callmekatootie')
...
const predictSuccess = async (params) => {
  const requestBody = createRequestBody(params.challenge)
